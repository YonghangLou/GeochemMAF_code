...
def generate_prediction_map_idw(
    *,
    ...
    possible_x_cols = ['Longitude', 'longitude', 'LONGITUDE', 'Lon', 'lon']
    possible_y_cols = ['Latitude', 'latitude', 'LATITUDE', 'Lat', 'lat']
    x_col = None
    y_col = None
    ...
    if x_col is None or y_col is None:
        ...
        if len(numeric_cols) >= 2:
            ...
            try:
                logger.warning(f'No standard coordinate columns were found. Using numeric columns {x_col} and {y_col} as coordinates')
            except Exception:
                pass
        else:
            try:
    ...
    cbar.set_label(_localize_text('Mineralization Potential Probability', lang=lang), fontsize=12)
...
class ResultOutputAgent(BaseAgent):
    ...
    def __init__(self, output_dir: str='./output', llm=None):
        role_description = (
            "Responsible for generating comprehensive geological reports and visualization outputs. "
            "Based on the results produced by all agents, create professional, clear, and interpretable "
            "deliverables together with meaningful geological interpretation and exploration suggestions."
        )
    ...
    def _display_model_name(self, model_key: Any) -> str:
        ...
        low = raw.lower()
        if low in {"som", "self organizing map", "self-organizing map", "selforganizingmap"}:
            return "SOM"
        return raw
    ...
    def _build_results_geological_interpretation_block(self, all_results: Dict[str, Any], reports_dir: str) -> str:
        ...
        if mineralization_text:
            lines.append(
                f"The recognized mineralization types are dominated by {mineralization_text}, suggesting a coupling relationship "
                "between the anomalous element associations and the potential ore-forming process."
            )
            lines.append("")
        if map_analysis:
    ...
    def _inject_into_results_section(self, report_content: str, section_md: str) -> str:
        ...
        for hdr in [r"\n##\s*3\.\s*Results\b", r"\n##\s*3\.\s*Results And Analysis\b", r"\n##\s*5\.\s*Mineral Potential Prediction\b", r"\n##\s*Results\b"]:
    ...
    def _output_feature_analysis_selection_doc(self, all_results: Dict[str, Any]) -> str:
        ...
        if best_model_name:
            ...
            model_type = "SOM (QE scoring, with optional probability calibration)" if best_low in {"som", "self organizing map", "self-organizing map", "selforganizingmap"} else "Supervised binary classification"
                    ...
                    anomalies_data.append(
                        {
                            ...
                            _localize_text('Number of Anomalous Samples', lang=lang): stats.get('high_value_count', stats.get('anomaly_count', 0)),
                            _localize_text('Anomalous Percentage (%)', lang=lang): stats.get('high_value_percentage', stats.get('anomaly_percentage', 0)),
                        }
                    )
        except TypeError:
            return ''
