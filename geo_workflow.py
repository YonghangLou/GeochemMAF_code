...
def _default_task_plan_markdown(target_deposit: str, available_agents: List[str]) -> str:
    ...
    return (
        ...
        f"## Stage 2: Geological Interpretation And Anomaly Recognition (geology_analysis)\n"
        f"- Goal: interpret anomalies and element associations using the metallogenic logic of {deposit}\n"
        f"- Deliverables: anomaly highlights, hypotheses on controlling factors, and target delineation suggestions\n\n"
...
def update_comprehensive_report_runtime(report_path: str, duration_seconds: float) -> bool:
    ...
    runtime_pattern = re.compile(r'^\\s*-\\s*\\*\\*Total Runtime\\*\\*:\\s*.*\\s*$')
    try:
...
class GeoChemistryWorkflow:
    def __init__(self, llm=None, output_dir: str='./output', progress_hook: Optional[Callable[[str, str, int, str], None]] = None):
        ...
        self.logger.info(f'{len(self.agents)} agents initialized in total')
    ...
    def _llm_governor_replan(self, node_name: str, state: dict) -> Dict[str, Any]:
        ...
        allowed_nodes = ["data_science_expert", "geology_analysis", "result_output"]
        prompt = (
        ...
        try:
            parsed = self.geology_expert_agent.decide_json(prompt, default_payload, config=state.get("config"))
    ...
    def _hitl_interpret(
        ...
        *,
        ...
        prompt = (
            ...
            "Return JSON with the following fields:\n"
            "- intent_summary: string, summarize your understanding of the user intent in 1-3 sentences\n"
            ...
            "- need_clarification: boolean, whether one key clarification is required before execution\n"
        ...
        *,
        ...
        for _ in range(max_rounds):
            if not user_text:
                ...
                if user_norm in {"continue", "run", "start", "ok", "y", "yes", "1"}:
            ...
            if not confirm_norm or confirm_norm in {"y", "yes", "1", "ok", "continue", "run", "start"}:
                return current_plan, {
                    "mode": "confirmed",
                    "user_text": user_text,
                    "parsed": parsed,
    ...
    def _hitl_sanitize_step_overrides(self, node_name: str, overrides: Any) -> Dict[str, Any]:
        ...
        for k, v in overrides.items():
            ...
            if key == "model_key":
                ...
                if raw in {"som", "self organizing map", "self-organizing map", "selforganizingmap"}:
                    out[key] = "som"
                continue
    ...
    def _hitl_interpret_step(
        ...
        *,
        ...
        prompt = (
            "You are the human-in-the-loop interpreter for a multi-agent system (HITL Step Interpreter)."
            ...
            "\n\n"
            "Strict constraints: you may only modify the method/content of the current step. "
            ...
            f"User input: {user_text}\n\n"
            "Return JSON with the following fields:\n"
            "- intent_summary: string, 1-3 sentences summarizing your understanding of the user's intent\n"
            ...
            "- need_clarification: boolean, whether one key point must be clarified before execution\n"
    ...
    def _maybe_hitl_dialog_for_node(self, *, state: dict, node_name: str, label: str, preview: str) -> tuple[str, Optional[Dict[str, Any]]]:
        ...
        for _ in range(max_rounds):
            ...
            if node_name == "data_science_expert":
                ...
                forced_model = None
                if "som" in user_norm2 or "self organizing map" in user_norm2 or "self-organizing map" in user_norm2:
                    forced_model = "som"
                if forced_model:
    ...
    def _maybe_hitl_override_decision(self, *, state: dict, context: dict, llm: Any, suggested_decision: str, valid_decisions: List[str]) -> tuple[str, Optional[Dict[str, Any]]]:
        ...
        for _ in range(max_rounds):
            ...
            confirm = _input_with_log_prefix(self.logger).strip()
            confirm_norm = confirm.strip().lower()
            if not confirm_norm or confirm_norm in {"y", "yes", "1", "ok"}:
    ...
    def _node_progress_range(self, node: str) -> Optional[tuple]:
        progress_map = {
            ...
            "result_output": (75, 95, "Result Output"),
    ...
    def _node_preview_text(self, node_name: str, state: dict, label: str) -> str:
        ...
        if node_name == "data_science_expert" and ("prediction model" in action or "predictions/scores" in action or "Train the missing model" in action):
    ...
    def _initialize_workflow(self, state: dict) -> dict:
        try:
            ...
            for agent in [self.data_science_expert_agent, self.geology_expert_agent, self.result_output_agent]:
                text = ""
                try:
            ...
            try:
                ...
                if _is_billing_error(e) or _is_auth_error(e):
                    ...
                    task_plan_final = _default_task_plan_markdown(str(target_deposit), list(self.agents.keys()))
                else:
            ...
            state['task_plan'] = [_default_task_plan_markdown(str(state.get("target_deposit_type") or "Unknown target deposit"), list(self.agents.keys()))]
        return state
