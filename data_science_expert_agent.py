...
def _maximum_youden_operating_point(y_true: Any, scores: Any) -> Dict[str, Any]:
    """Compute the full ROC table and select the highest finite threshold at max J."""
...
class DataScienceExpertAgent(BaseAgent):
    ...
    def _plot_confusion_matrix_heatmap(
        self,
        *,
        cm: np.ndarray,
        out_path: str,
        ...
        try:
            ...
            with plt.rc_context(style):
                ...
                annot_kws = {"fontsize": float(font_size)} if font_size is not None else None
                sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", cbar=False, ax=ax, linewidths=0.5, linecolor="white", annot_kws=annot_kws)
                ...
                fig.tight_layout()
                fig.savefig(out_path, dpi=300 if font_size is not None else 220, bbox_inches="tight", facecolor="white", format="png")
                plt.close(fig)
            return out_path
        except Exception as e:
            try:
    ...
    def run(self, state: dict) -> dict:
        ...
        try:
            ...
            geology_expert_enabled = bool(config.get('geology_expert_enabled', True))
            ...
            if geology_expert_results or not geology_expert_enabled:
                ...
                if target_elements:
                    ...
                    if filtered_feature_cols:
                        ...
                        state['feature_cols'] = feature_cols
                state['model_input_metadata'] = {
                    'geology_expert_enabled': bool(geology_expert_enabled),
                    'feature_input_mode': 'geology_selected_elements' if geology_expert_enabled else 'all_valid_geochemical_elements',
        ...
        *,
        ...
        try:
            model_results["_dataset_info"] = {
                ...
                "training_sampling_plan": sampling_plan,
                "qe_probability_calibration": bool((config or {}).get("som_qe_calibration_enabled", True)),
            }
        except Exception:
            pass
        ...
        try:
            ...
            if x_col and y_col and x_col in som_input_data.columns and y_col in som_input_data.columns:
                ...
                try:
                    ...
                    if geology_expert_enabled and isinstance(som_filtered_elements_analysis, dict):
    ...
    def _train_and_evaluate_models(
        ...
        *,
        ...
        qe_calibration_enabled: bool = True,
    ) -> Dict[str, Any]:
        ...
        estimator.set_params(calibrate=bool(qe_calibration_enabled))
        ...
        try:
            ...
            if isinstance(best_details, dict):
                ...
                predict_cm = None
                predict_cm_youden = None
                full_region_youden = None
                try:
                    ...
                    if bool(np.any(mask_all)):
                        ...
                        if scores_all is not None:
                            ...
                            if s_all.size == y_all_arr.size:
                                ...
                                scores_all_valid = np.asarray(s_all)[mask_all]
                                preds_all = (scores_all_valid >= 0.5).astype(int)
                                predict_cm = confusion_matrix(y_true_all, preds_all).tolist()
                                full_region_youden = _maximum_youden_operating_point(y_true_all, scores_all_valid)
                                threshold_youden = float(full_region_youden["threshold"])
                                preds_all_youden = (full_region_youden["scores"] >= threshold_youden).astype(int)
                                predict_cm_youden = confusion_matrix(full_region_youden["y_true"], preds_all_youden, labels=[0, 1]).tolist()
                except Exception:
                    predict_cm = None
                    predict_cm_youden = None
                    full_region_youden = None
                ...
                if predict_cm_youden is not None and isinstance(full_region_youden, dict):
                    try:
                        cm_youden_path = os.path.join(viz_dir, "best_model_predict_confusion_matrix_youden.png")
                        artifacts["predict_confusion_matrix_youden_path"] = self._plot_confusion_matrix_heatmap(
                            cm=np.asarray(predict_cm_youden),
                            out_path=cm_youden_path,
                            title=f"{model_display} Predict Confusion Matrix (Youden)",
                            font_size=24,
                        )
                        roc_table = full_region_youden.get("roc_table")
                        roc_table_path = os.path.join(viz_dir, "best_model_full_region_youden_roc_thresholds.csv")
                        if isinstance(roc_table, pd.DataFrame):
                            _atomic_write_csv(roc_table, roc_table_path, index=False, encoding="utf-8-sig")
                            artifacts["full_region_youden_roc_thresholds_path"] = os.path.abspath(roc_table_path)
                        cm_youden_csv = os.path.join(viz_dir, "best_model_predict_confusion_matrix_youden.csv")
                        cm_youden_df = pd.DataFrame(
                            np.asarray(predict_cm_youden, dtype=int),
                            index=pd.Index([0, 1], name="true_label"),
                            columns=["predicted_0", "predicted_1"],
                        )
                        _atomic_write_csv(cm_youden_df, cm_youden_csv, index=True, encoding="utf-8-sig")
                        artifacts["predict_confusion_matrix_youden_csv"] = os.path.abspath(cm_youden_csv)
                        youden_summary = {
                            "scope": "full_region_valid_labels",
                            "score": "calibrated_qe_probability" if bool(qe_calibration_enabled) else "normalized_qe_score",
                            "threshold": float(full_region_youden["threshold"]),
                            "youden_index": float(full_region_youden["youden_index"]),
                            "sensitivity": float(full_region_youden["sensitivity"]),
                            "specificity": float(full_region_youden["specificity"]),
                            "confusion_matrix": predict_cm_youden,
                            "caution": "The threshold and confusion matrix use the same full-region labels and are retrospective, not held-out estimates.",
                        }
                        youden_json = os.path.join(viz_dir, "best_model_full_region_youden_metrics.json")
                        _atomic_write_text(youden_json, json.dumps(youden_summary, ensure_ascii=False, indent=2) + "\n")
                        artifacts["full_region_youden_metrics_path"] = os.path.abspath(youden_json)
...
def plot_youden_prospecting_target_map(
    *,
    ...
    point_data = pd.DataFrame(
        {
            "Longitude": lon[valid],
            "Latitude": lat[valid],
            "label": truth_arr[:n][valid].astype(int),
            "qe_score": score_arr[:n][valid].astype(float),
        }
    )
    operating = _maximum_youden_operating_point(truth_arr[:n], score_arr[:n])
    threshold = float(operating["threshold"])
    youden_index = float(operating["youden_index"])
    point_data["predicted_class_youden"] = (point_data["qe_score"].to_numpy() >= threshold).astype(int)
    point_data["youden_threshold"] = threshold

    from scipy.spatial import cKDTree

    xy = point_data[["Longitude", "Latitude"]].to_numpy(dtype=float)
    gx = np.linspace(float(np.min(xy[:, 0])), float(np.max(xy[:, 0])), 300)
    gy = np.linspace(float(np.min(xy[:, 1])), float(np.max(xy[:, 1])), 300)
    xx, yy = np.meshgrid(gx, gy)
    _, nearest = cKDTree(xy).query(np.column_stack([xx.ravel(), yy.ravel()]), k=1)
    surface = point_data["predicted_class_youden"].to_numpy(dtype=np.uint8)[nearest].reshape(300, 300)

    width_px, height_px, dpi = 3459, 1636, 300
    style = {
    ...
    with plt.rc_context(style):
        ...
        ax = fig.add_axes([341 / width_px, 256 / height_px, 2650 / width_px, 1256 / height_px])
        cax = fig.add_axes([3048 / width_px, 256 / height_px, 120 / width_px, 1256 / height_px])
        cmap = ListedColormap(["#172a88", "#d73027"])
        norm = BoundaryNorm([-0.5, 0.5, 1.5], cmap.N)
        extent = [float(gx[0]), float(gx[-1]), float(gy[0]), float(gy[-1])]
        image = ax.imshow(surface, extent=extent, origin="lower", aspect="equal", interpolation="nearest", cmap=cmap, norm=norm)
        deposits = point_data.loc[point_data["label"].eq(1), ["Longitude", "Latitude"]].to_numpy(dtype=float)
        if deposits.size:
            ax.scatter(
                deposits[:, 0],
                deposits[:, 1],
            ...
            ax.legend(loc="lower left", scatterpoints=1, handlelength=0.9, handletextpad=0.35, borderpad=0.35, labelspacing=0.3, markerscale=0.9)
        ax.text(
            0.025,
            0.955,
            f"Youden={youden_index:.4f}",
        ...
        ax.set(title="Prospecting Target Map", xlabel="Longitude", ylabel="Latitude", xlim=extent[:2], ylim=extent[2:])
        colorbar = fig.colorbar(image, cax=cax, ticks=[0, 1])
        colorbar.set_label("Classification", fontsize=24)
        fig.canvas.draw()
        map_position = ax.get_position()
        cax.set_position([3048 / width_px, map_position.y0, 120 / width_px, map_position.height])
        fig.savefig(output_abs, dpi=dpi, facecolor="white")
        plt.close(fig)

    stem = os.path.splitext(output_abs)[0]
    source_csv = stem + "_source.csv"
    roc_csv = stem + "_roc_thresholds.csv"
    metrics_json = stem + "_metrics.json"
    _atomic_write_csv(point_data, source_csv, index=False, encoding="utf-8-sig")
    roc_table = operating["roc_table"]
    _atomic_write_csv(roc_table, roc_csv, index=False, encoding="utf-8-sig")
    predictions = (operating["scores"] >= threshold).astype(int)
    cm = confusion_matrix(operating["y_true"], predictions, labels=[0, 1])
    metrics_payload = {
        "scope": "full_region_valid_labels",
        "score": "spatially_filtered_qe_score",
        "threshold": threshold,
        "youden_index": youden_index,
        "sensitivity": float(operating["sensitivity"]),
        "specificity": float(operating["specificity"]),
        "confusion_matrix": cm.tolist(),
        "coordinate_conversion": coordinate_meta,
        "map_rule": "QE threshold classification followed by nearest-neighbor categorical display",
    ...
    return {
        **metrics_payload,
        ...
        "roc_thresholds_csv": os.path.abspath(roc_csv),
...
def run_mineral_qe_analysis_from_df(
    *,
    ...
    try:
        if label_col is not None and label_col in df.columns:
            ...
            if bool(np.any(valid_mask)):
                ...
                if class_count >= 2:
                    ...
                    y_true_full = np.full(len(y_true), np.nan, dtype=float)
                    y_true_full[valid_mask] = y_true_bin
                    target_map_path = os.path.join(out_dir, "prospecting_target_map_youden.png")
                    youden_target_map = plot_youden_prospecting_target_map(
                        raw_data=plot_df,
                        qe_scores=qe_scores,
                        y_true=y_true_full,
                        output_path=target_map_path,
                    )
                    youden_target_map["enabled"] = True
                    youden_target_map["error"] = ""
                    roc_result = {
                        "enabled": True,
                        "error": "",
                        "source": "label_col",
                        ...
                        "youden_threshold": float(youden_target_map["threshold"]),
                        "youden_index": float(youden_target_map["youden_index"]),
        ...
        youden_target_map = {"enabled": False, "error": str(e), "map_path": ""}
    try:
        if not bool(roc_result.get("enabled")) and mineral_shp_path and os.path.exists(str(mineral_shp_path)) and os.path.exists(output_arcgis):
    ...
    return {
        ...
        "mineral_elements": mineral_elements,
        "best_params": best_params,
        "threshold": youden_target_map.get("threshold") if bool(youden_target_map.get("enabled")) else None,
        ...
        "roc": roc_result,
        "youden_target_map": youden_target_map,
        "target_delineation": youden_target_map,
